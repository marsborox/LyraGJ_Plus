...
public class RoomSpawner : MonoBehaviour
{
    ...
    public  Room SpawnRoom(Room inputRoom, Direction direction, List<Room> roomPrefabList,List<Room>deadEndPrefabList,ref List<Room>roomList, Room[,] roomGrid,int roomSize,ref int spawnedRoomCounter)
    {
        ...
        int xOfSpawn;
        int yOfSpawn;
        //nejak vyriesit boundaries gridu
        //asi po obvode spawnuť steny)
        ...
        //rooms set above
        GetRoomByCoord(xOfSpawn - 1, yOfSpawn,roomGrid, out neighborLeft);
        GetRoomByCoord(xOfSpawn + 1, yOfSpawn, roomGrid, out neighborRight);
        GetRoomByCoord(xOfSpawn, yOfSpawn + 1, roomGrid, out neighborTop);
        GetRoomByCoord(xOfSpawn, yOfSpawn - 1, roomGrid, out neighborBottom);
        ...
        Room pickedRoomPrefab;
        if (returnList.Count == 0)
        {
            pickedRoomPrefab = ReturnViableDeadEnd(deadEndPrefabList,direction);
        }
        else
        { 
            pickedRoomPrefab = returnList[randomRoomIndex];
        }

        Vector3 spawnPosition = this.transform.position;

        //spawn room
        spawnPosition = inputRoom.transform.position + ReturnRoomSpawnPosition(inputRoom, direction,roomSize);
        ...
        //disable triggers of neighboring tiles  to new room 
    ...
    private void CheckNeighbors(int x, int y, Room[,] roomGrid)
    {
        bool left = IsGridSlotUsedBool(x - 1, y, roomGrid);
        bool right = IsGridSlotUsedBool(x + 1, y, roomGrid);
        bool top = IsGridSlotUsedBool(x, y + 1, roomGrid);
        bool bottom = IsGridSlotUsedBool(x, y - 1, roomGrid);

        string spawnString = top.ToString() + left.ToString() + bottom.ToString() + right.ToString();
        Debug.Log(spawnString);
    }
    private bool IsGridSlotUsedBool(int x, int y, Room[,] roomGrid)
    {
    ...
    private void GetRoomByCoord(int x, int y, Room[,] roomGrid, out Room room)
    {
        room = roomGrid[y, x];
    }
    ...
    private Room ReturnViableDeadEnd(List <Room> deadEndPrefabList,Direction direction)
    { 
        Room room= new Room();
        List<Room> returnList = new List<Room>();
        foreach (Room deadEnd in deadEndPrefabList)
        {
            bool examinedDoor = false;
            switch (direction)
            {
                case Direction.LEFT: { examinedDoor = deadEnd.rightDoor; } break;
                case Direction.RIGHT: { examinedDoor = deadEnd.leftDoor; } break;
                case Direction.UP: { examinedDoor = deadEnd.bottomDoor; } break;
                case Direction.DOWN: { examinedDoor = deadEnd.topDoor; } break;
                default: {/*Debug.Log("Unknown direction");*/break; }
            }
            if (examinedDoor)
            { 
                returnList.Add(deadEnd);
            }
        }
        int roomIndex = UnityEngine.Random.Range(0, returnList.Count - 1);
        room = returnList[roomIndex];
        return room;
    }
    private Vector3 ReturnRoomSpawnPosition(Room room, Direction direction,int roomSize)
    {
    ...
    public void TestArrayContent(Room[,] roomGrid,int roomGridSize)
    {
        int ammountOfNull = 0;
        int ammountOfUsed = 0;
    ...
    private void PrintAllRoomNamesInList(List<Room> roomList, string listName)
    {
